...
int main()
{
	int *p,*bt,*bt1,*at,*oz,*qu,*pro; int i,j,k,l,t,fr,re,z; int tl;float avg=0.0; int wt;
	int check,c;
	FILE *pt;
	pt=fopen("priopr.txt","r");
	if(pt==NULL)
	{
		printf("Error\n");
		exit(1);
	...
	for(j=0;j<t;j++)
	{
		...
		avg+=wt;
	printf("%d\t%d\t%d\t%d\t%d\t%d\n",p[j],at[j],bt[j],pro[j],wt,(bt[j]+wt));
	}
